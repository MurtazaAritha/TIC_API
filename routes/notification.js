...
const { getNotificationsService, updateNotificationService } = require('../service/notification_service');
...
router.post('/api/v1/notifications/markAsRead', async (req, res) => {
  try {
    ...
    if (isValid && notifications && notifications.length > 0 ) {
      let response = await updateNotificationService({ user_id, notifications });
      if (response) {
        responseType = SUCCESS;
        statusCode = STATUS_CODE_SUCCESS;
